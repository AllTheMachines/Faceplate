...
export function generateBindingsJS(
  ...
  const knobSetups = knobs
    .map((knob) => {
      ...
      return `    setupKnobInteraction('${toKebabCase(knob.name)}', '${knob.parameterId}', ${defaultValue});`
    })
    .join('\n')
  ...
  const sliderSetups = sliders
    .map((slider) => {
      ...
      return `    setupSliderInteraction('${toKebabCase(slider.name)}', '${slider.parameterId}', ${defaultValue});`
    })
    .join('\n')
...
${buttonSetups || '        // No buttons with parameter bindings'}
  ...
  bridge = {
    setParameter: (paramId, value) => console.log(\`[Mock] setParameter(\${paramId}, \${value})\`),
    getParameter: (paramId) => Promise.resolve(0.5),
    beginGesture: (paramId) => console.log(\`[Mock] beginGesture(\${paramId})\`),
    endGesture: (paramId) => console.log(\`[Mock] endGesture(\${paramId})\`)
  };
}
