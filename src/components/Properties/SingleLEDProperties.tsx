import { SingleLEDElementConfig, ElementConfig } from '../../types/elements'
import { NumberInput, ColorInput, PropertySection } from './'

interface SingleLEDPropertiesProps {
  element: SingleLEDElementConfig
  onUpdate: (updates: Partial<ElementConfig>) => void
}

export function SingleLEDProperties({ element, onUpdate }: SingleLEDPropertiesProps) {
  return (
    <>
      {/* State */}
      <PropertySection title="State">
        <div>
          ...
          <select
            value={element.state}
            onChange={(e) => onUpdate({ state: e.target.value as 'on' | 'off' })}
      ...
      <PropertySection title="Glow">
        <label
          htmlFor="singleled-glow-enabled"
          className="flex items-center gap-2 cursor-pointer select-none"
        >
          <input
            type="checkbox"
            id="singleled-glow-enabled"
