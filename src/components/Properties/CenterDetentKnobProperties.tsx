import { CenterDetentKnobElementConfig, ElementConfig } from '../../types/elements'
import { NumberInput, ColorInput, PropertySection, TextInput } from './'
import { useStore } from '../../store'
import { RotaryLayers } from '../../types/elementStyle'

interface CenterDetentKnobPropertiesProps {
  element: CenterDetentKnobElementConfig
  onUpdate: (updates: Partial<ElementConfig>) => void
}

export function CenterDetentKnobProperties({ element, onUpdate }: CenterDetentKnobPropertiesProps) {
  ...
  return (
    <>
      ...
      {/* Center Detent Configuration */}
      <PropertySection title="Center Detent">
        <NumberInput
          label="Snap Threshold"
          value={element.snapThreshold}
          onChange={(snapThreshold) => onUpdate({ snapThreshold })}
          min={0.01}
          max={0.2}
          step={0.01}
        />
        <label className="flex items-center gap-2 cursor-pointer select-none">
          <input
            type="checkbox"
            checked={element.showCenterMark}
            onChange={(e) => onUpdate({ showCenterMark: e.target.checked })}
            className="rounded border-gray-600 bg-gray-700 text-blue-500 focus:ring-blue-500"
          />
          <span className="text-sm text-gray-300">Show Center Mark</span>
        </label>
      </PropertySection>
          ...
          <>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  onUpdate({ labelPosition: e.target.value as CenterDetentKnobElementConfig['labelPosition'] })
                }
                className="w-full bg-gray-700 border border-gray-600 text-white rounded px-2 py-1.5 text-sm"
              >
          ...
          <>
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  onUpdate({ valuePosition: e.target.value as CenterDetentKnobElementConfig['valuePosition'] })
                }
                className="w-full bg-gray-700 border border-gray-600 text-white rounded px-2 py-1.5 text-sm"
              >
                <option value="top">Top</option>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  onUpdate({ valueFormat: e.target.value as CenterDetentKnobElementConfig['valueFormat'] })
                }
                className="w-full bg-gray-700 border border-gray-600 text-white rounded px-2 py-1.5 text-sm"
              >
