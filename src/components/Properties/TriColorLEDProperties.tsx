import { TriColorLEDElementConfig, ElementConfig } from '../../types/elements'
import { NumberInput, ColorInput, PropertySection } from './'

interface TriColorLEDPropertiesProps {
  element: TriColorLEDElementConfig
  onUpdate: (updates: Partial<ElementConfig>) => void
}

export function TriColorLEDProperties({ element, onUpdate }: TriColorLEDPropertiesProps) {
  return (
    <>
      {/* State */}
      <PropertySection title="State">
        <div>
          ...
          <select
            value={element.state}
            onChange={(e) => onUpdate({ state: e.target.value as 'off' | 'yellow' | 'red' })}
      ...
      <PropertySection title="Glow">
        <label
          htmlFor="tricolorled-glow-enabled"
          className="flex items-center gap-2 cursor-pointer select-none"
        >
          <input
            type="checkbox"
            id="tricolorled-glow-enabled"
      ...
      <PropertySection title="Color Palette">
        ...
        {element.colorPalette === 'custom' && (
          <>
            ...
            <ColorInput
              label="Yellow Color"
              value={element.yellowColor}
              onChange={(yellowColor) => onUpdate({ yellowColor })}
            />
            <ColorInput
              label="Red Color"
