...
4. Visual verification - all 9 palette items show correct previews:
   - Knob (standard circular)
   - Arc Knob (arc style)
   - V Slider (vertical orientation, taller than wide)
   - H Slider (horizontal orientation, wider than tall)
   - Momentary (BTN label)
   - Toggle (TGL label)
   - Label (Aa text)
   - Meter (vertical bar)
   - Image (placeholder icon)
5. Drag each palette item to canvas - verify element created (no silent failures)
  </verify>
