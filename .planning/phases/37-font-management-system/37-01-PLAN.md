...
must_haves:
  ...
  key_links:
    ...
    - from: "src/services/fonts/fontManager.ts"
      ...
      via: "imports directoryOpen for file system access"
      pattern: "import.*directoryOpen.*browser-fs-access"
---
...
import { storeFont, getAllFonts, clearFonts, storeDirectoryHandle, getDirectoryHandle, clearDirectoryHandle } from './fontStorage'
...
```
  ...
  <done>
    ...
    - index.ts re-exports all font service modules including getAllFonts
    - browser-fs-access directoryOpen explicitly imported
...
- fontManager.ts explicitly imports directoryOpen from browser-fs-access
