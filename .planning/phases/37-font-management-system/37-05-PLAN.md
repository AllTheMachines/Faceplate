...
**Scope boundary:** Update only codeGenerator.ts and previewExport.ts. If changes cascade beyond these two files, STOP and report for additional planning.
...
**If functions calling it need to become async:** Update their signatures and verify callers are already async (they should be for export operations). If any callers are NOT async and cannot easily be made async, STOP and report - do not cascade changes into UI components or event handlers.
  </action>
  <verify>
    - `npm run build` completes without errors
  ...
  <done>
    ...
    - Changes limited to codeGenerator.ts and previewExport.ts only
  </done>
</task>
