...
@src/App.tsx
@src/components/Palette/PaletteItem.tsx
@.planning/phases/13-extended-elements/13-15-SUMMARY.md
</context>
...
<task type="auto">
  ...
  <action>
1. Import DragOverlay and DragStartEvent from @dnd-kit/core:
```typescript
import {
  DndContext,
  DragEndEvent,
...
2. Import useState if not already present (check existing React imports first, add to existing import statement if React is already imported):
```typescript
// If React is imported as: import React from 'react'
// Change to: import React, { useState } from 'react'
// Or if using: import { ... } from 'react'
...
4. Update handleDragEnd to clear the drag data. Add this AFTER the centering logic from Plan 13-15 (which subtracts width/2 and height/2), at the very end of the handleDragEnd function:
```typescript
// At the end of handleDragEnd, AFTER the centering logic and addElement call, clear drag state
setActiveDragData(null)
```

5. Add onDragStart to DndContext:
```typescript
<DndContext
  sensors={sensors}
