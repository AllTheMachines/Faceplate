...
must_haves:
  truths:
    - "User can import a style for any category (rotary, linear, arc, button, meter)"
    - "User can rename and delete styles in ManageElementStylesDialog"
    - "Layer hover highlighting works during mapping - hovering a row dims other layers"
    - "Asset sidebar import button opens ElementLayerMappingDialog with category selection"
    - "All PropertyPanel components show Manage styles button and open correct category dialog"
  artifacts: []
  key_links: []
---
...
- ElementStyleSection shared component used by all property panels
- "Manage styles..." button in all styled element PropertyPanels
- Thumbnail preview for selected styles
- Asset sidebar import button (Entry Point #1)
  </what-built>
  <how-to-verify>
1. Open http://localhost:5173

2. **Test Asset sidebar import (Entry Point #1 from CONTEXT.md):**
   a. In left sidebar, find Asset section
   b. Click "Import SVG Style" button
   c. ElementLayerMappingDialog should open
   d. Should prompt for category selection first (since opened without category)
   e. Select a category and continue with import flow

3. **Test Slider style management:**
...
4. **Test Button style management:**
...
<verification>
1. User has tested Asset sidebar import flow
2. User has tested slider, button, and meter style management flows
3. Layer hover highlighting works in ElementLayerMappingDialog
4. Import and re-map flows complete without errors
5. Thumbnail previews display correctly
</verification>
...
- All 3 entry points from CONTEXT.md work:
  - Entry Point #1: Asset sidebar import button
  - Entry Point #2: PropertyPanel style dropdown
  - Entry Point #3: Covered via Manage styles button in left sidebar properties
