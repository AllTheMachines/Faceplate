...
<objective>
Add visible undo/redo buttons to the toolbar and implement keyboard layout detection for correct shortcut label display.

Purpose: Users currently rely on keyboard shortcuts (Ctrl+Z/Y) for undo/redo with no visual feedback. Adding buttons makes undo/redo discoverable and shows current state. Layout detection ensures tooltips show correct shortcut labels for the user's keyboard.
...
```
  ...
  <done>Keyboard layout detection utility created</done>
</task>
...
5. Keyboard layout detected for correct shortcut labels
  ...
  <verify>
Run build and verify buttons appear, then test keyboard shortcuts on QWERTZ:
```bash
npm run build
npm run dev
# Manually verify:
...
# 6. Test Ctrl+Z shortcut works (add element, press Ctrl+Z, element removed)
# 7. Test Ctrl+Y shortcut works (press Ctrl+Y after undo, element restored)
```

**QWERTZ keyboard shortcut verification:**
To verify shortcuts work on QWERTZ layouts (UX-02 requirement):
1. Open browser DevTools console
2. Check that react-hotkeys-hook binds to KeyboardEvent.code (physical key), not KeyboardEvent.key (character)
3. Run: `document.addEventListener('keydown', e => console.log('code:', e.code, 'key:', e.key))`
4. Press Ctrl+Z on any keyboard - should log `code: KeyZ` regardless of layout
5. The shortcut will work because it uses the physical key position, not the character output

Alternatively, if you have access to a QWERTZ system:
1. Add an element to the canvas
2. Press Ctrl+Z (physical Z key position, which outputs 'y' on QWERTZ)
3. Verify element is undone
4. Press Ctrl+Y (physical Y key position, which outputs 'z' on QWERTZ)
5. Verify element is redone
  </verify>
  <done>Undo/redo buttons visible in toolbar with correct state, tooltips, and keyboard shortcuts working on QWERTY/QWERTZ/AZERTY layouts</done>
</task>
...
7. Keyboard shortcuts (Ctrl+Z, Ctrl+Y) work on all layouts
8. Tooltips show correct shortcut labels
9. **QWERTZ verification**: Shortcuts use KeyboardEvent.code (physical key), confirmed in DevTools
</verification>
...
- [ ] Ctrl+Z/Ctrl+Y shortcuts work (verified via DevTools showing KeyboardEvent.code binding)
</success_criteria>
