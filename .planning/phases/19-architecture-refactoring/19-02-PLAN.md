...
Read the current Element.tsx first to preserve existing selection handling logic, then refactor to use the registry created in Task 1.

The updated Element.tsx should:

1. **Remove all 25 individual renderer imports** - These are now handled by the registry in renderers/index.ts

2. **Add new import for getRenderer:**
```typescript
import { getRenderer } from './renderers'
```

3. **Remove the renderContent() function** with the switch statement entirely
...
// Fallback for unknown types (defensive - should never happen)
if (!Renderer) {
...
6. **Keep all selection handling logic unchanged** - The handleClick function with shift/ctrl/cmd modifiers should remain exactly as-is

Key changes summary:
- Remove ~25 renderer import statements
- Remove renderContent() function (~80 lines)
- Add 1 import: getRenderer from './renderers'
- Add 5 lines for registry lookup + fallback check
- File should shrink from ~135 LOC to ~60 LOC
  </action>
  <verify>
