...
must_haves:
  ...
  key_links:
    - from: "src/components/History/HistoryPanel.tsx"
      ...
      via: "jumpToHistoryIndex function using imperative getState() calls"
      pattern: "undo\\(|redo\\("
    ...
    - from: "src/components/History/HistoryPanel.tsx"
      ...
      via: "useStore from zustand with temporal selector for reactive display"
...
3. Update HistoryPanel.tsx with TWO DISTINCT PATTERNS for temporal state:

   **PATTERN A - Reactive subscriptions (for displaying state in UI):**
   Use `useStore(useAppStore.temporal, selector)` to subscribe to state changes.
   This triggers re-renders when the subscribed slice changes.
   ```typescript
   import { useStore } from 'zustand'
   import { useStore as useAppStore } from '../../store'

   // These re-render the component when pastStates/futureStates change
   ...
   **PATTERN B - Imperative calls (for triggering actions):**
   Use `useAppStore.temporal.getState()` to get current state and call undo/redo.
   This does NOT trigger re-renders - it's a one-time read for action dispatch.
   ```typescript
   const jumpToHistoryIndex = (targetIndex: number) => {
     // Imperative: get current state snapshot, then call undo/redo
     const { pastStates, undo, redo } = useAppStore.temporal.getState()
     const currentIndex = pastStates.length

     if (targetIndex < currentIndex) {
       undo(currentIndex - targetIndex)  // undo N steps
     } else if (targetIndex > currentIndex) {
       redo(targetIndex - currentIndex)  // redo N steps
     }
   }
   ```

   - Define jumpToHistoryIndex function inside HistoryPanel.tsx (not in HistoryEntry)
...
<task type="auto">
  <name>Task 3: Polish visual design and verify success criteria</name>
  <files>
    src/components/History/HistoryPanel.tsx
    src/components/History/HistoryEntry.tsx
  </files>
...
<task type="auto">
  <name>Task 4: Update build timestamp</name>
  <files>
    src/buildInfo.ts
  </files>
  <action>
Update src/buildInfo.ts with current timestamp in format: 'DD Mon HH:MM CET'

This is required per CLAUDE.md instructions when finishing code changes.
  </action>
  <verify>
    src/buildInfo.ts contains current timestamp
  </verify>
  <done>
    Build timestamp updated to reflect phase completion
  </done>
</task>
