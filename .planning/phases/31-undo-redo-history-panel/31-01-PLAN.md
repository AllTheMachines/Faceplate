...
must_haves:
  ...
  artifacts:
    - path: "src/hooks/useTemporalStore.ts"
      provides: "Reactive hook for accessing pastStates/futureStates"
      exports: ["useTemporalStore"]
  ...
  key_links:
    - from: "src/components/History/HistoryPanel.tsx"
      to: "useTemporalStore"
      via: "hook import and subscription"
...
<task type="auto">
  <name>Task 2: Create reactive temporal hook</name>
  <files>
    src/hooks/useTemporalStore.ts
    src/hooks/useHistoryPanel.ts
  </files>
  <action>
1. Create src/hooks/useTemporalStore.ts:
   - Import useStoreWithEqualityFn from 'zustand/traditional'
   - Import type TemporalState from 'zundo'
   - Import useStore from '../store'
   - Define TrackedState type matching what's partialized in store (elements, canvasWidth, canvasHeight, backgroundColor, assets, knobStyles)
   - Export useTemporalStore function that wraps useStoreWithEqualityFn(useStore.temporal, selector, equality)
...
2. Create src/hooks/useHistoryPanel.ts:
   - Import useState from 'react'
   - Create and export useHistoryPanel hook that manages:
     - isPanelVisible: boolean state (default false)
  ...
  <verify>
    TypeScript compiles without errors (npm run build should not fail on these files)
    Hook files exist in src/hooks/
  </verify>
  <done>
    useTemporalStore hook provides reactive access to pastStates/futureStates
    useHistoryPanel hook manages panel visibility state
  </done>
</task>

<task type="auto">
  <name>Task 3: Create HistoryPanel and HistoryEntry components</name>
  <files>
    src/components/History/HistoryPanel.tsx
    src/components/History/HistoryEntry.tsx
    src/components/History/index.ts
  ...
  <action>
1. Create src/components/History/HistoryEntry.tsx:
...
2. Create src/components/History/HistoryPanel.tsx:
...
3. Create src/components/History/index.ts:
   - Export { HistoryPanel } from './HistoryPanel'
   - Export { HistoryEntry } from './HistoryEntry'

4. Update App.tsx to import and render HistoryPanel in the bottom Panel instead of placeholder
  </action>
  <verify>
...
- useTemporalStore hook provides reactive history access
- HistoryPanel shows scrollable list of state changes
