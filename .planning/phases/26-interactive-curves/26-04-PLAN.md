...
must_haves:
  ...
  key_links:
    ...
    - from: "src/components/Properties/index.ts"
      to: "EQCurveProperties"
      via: "propertyPanelRegistry entry"
      pattern: "eqcurve.*EQCurveProperties"
---
...
- Register all 5 property panels in propertyPanelRegistry:
  ```typescript
  propertyPanelRegistry.set('eqcurve', EQCurveProperties)
  propertyPanelRegistry.set('compressorcurve', CompressorCurveProperties)
  propertyPanelRegistry.set('envelopedisplay', EnvelopeDisplayProperties)
  propertyPanelRegistry.set('lfodisplay', LFODisplayProperties)
  propertyPanelRegistry.set('filterresponse', FilterResponseProperties)
  ```

Phase 19 established property panel registry as the standard pattern for element configuration UI. All curve elements must be explicitly registered to appear in the properties sidebar when selected.
  </action>
  <verify>
1. npx tsc --noEmit passes
2. All 5 property panels exist in src/components/Properties/curves/
3. Property panel registry includes all 5 curve types:
   - grep 'eqcurve.*EQCurveProperties' src/components/Properties/index.ts
   - grep 'compressorcurve.*CompressorCurveProperties' src/components/Properties/index.ts
   - grep 'envelopedisplay.*EnvelopeDisplayProperties' src/components/Properties/index.ts
   - grep 'lfodisplay.*LFODisplayProperties' src/components/Properties/index.ts
   - grep 'filterresponse.*FilterResponseProperties' src/components/Properties/index.ts
  </verify>
  <done>Property panels for all 5 curve types with full configuration options and explicit registry registration</done>
</task>
...
5. propertyPanelRegistry includes all 5 curve element types
6. Palette includes "Curves" category with 5 items
7. `npm run dev` shows Curves in palette (visual check)
</verification>
...
- Property panels are registered and appear when curve elements are selected
