...
2. Validate generated HTML structure for each curve type:
   - grep 'generateEQCurveHTML\|generateCompressorCurveHTML\|generateEnvelopeDisplayHTML\|generateLFODisplayHTML\|generateFilterResponseHTML' src/services/export/htmlGenerator.ts
3. Check JavaScript syntax in generated output:
   - Create test element and inspect generated HTML for valid script tags
   - Verify Canvas element present: grep '<canvas' on generated output
4. Verify JUCE event listeners registered for all 5 types:
   - grep "addEventListener.*eqData\|addEventListener.*compressorData\|addEventListener.*envelopeData\|addEventListener.*lfoData\|addEventListener.*filterData" src/services/export/htmlGenerator.ts
5. Validate inline JavaScript structure:
   - Check window.devicePixelRatio usage for HiDPI scaling
   - Verify update functions exposed globally (window.updateEQCurve_, etc.)
  </verify>
...
6. Generated JavaScript has valid syntax (no template literal errors)
7. Canvas elements properly structured in HTML output
</verification>
