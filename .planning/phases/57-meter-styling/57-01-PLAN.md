...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/types/elements/displays.ts"
      provides: "styleId on BaseProfessionalMeterConfig"
...
<objective>
Add SVG styling type support to professional meters by extending MeterLayers with zone fill layers and adding styleId/colorOverrides to BaseProfessionalMeterConfig.

Purpose: Enable all 24 professional meter types to use SVG styles with multi-zone color fills (green/yellow/red) following user decisions from CONTEXT.md.
...
<task type="auto">
  <name>Task 2: Add styleId and colorOverrides to BaseProfessionalMeterConfig</name>
  <files>src/types/elements/displays.ts</files>
  <action>
Update BaseProfessionalMeterConfig to include SVG styling properties:
...
2. Add styleId and colorOverrides to BaseProfessionalMeterConfig:
```typescript
...
- Placed near end of interface, before closing brace
- All 24 professional meter types inherit these properties automatically
  </action>
  <verify>Run `npx tsc --noEmit` - should compile without errors; grep for 'styleId' in displays.ts shows it in BaseProfessionalMeterConfig</verify>
  <done>BaseProfessionalMeterConfig has styleId and colorOverrides properties; all 24 meter types inherit them</done>
</task>
...
3. BaseProfessionalMeterConfig has 'styleId?: string' and 'colorOverrides?: ColorOverrides'
...
- All 24 professional meter types have styleId and colorOverrides available
