...
must_haves:
  truths:
    ...
    - "Elements created from palette have isPro field set automatically"
    - "Elements loaded from project JSON have isPro field set automatically"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/App.tsx"
      ...
      via: "import and call"
      pattern: "isProElement.*element\\.type"
    - from: "src/services/serialization.ts"
      to: "src/services/proElements.ts"
      via: "import and call"
      pattern: "isProElement.*el\\.type"
---

<objective>
Create the data layer for Pro element feature gating: registry of 43 Pro element types, license state in Zustand with localStorage persistence, useLicense hook for component access, and automatic isPro field population on element creation and project load.

Purpose: Establishes the foundation for Pro/Free element classification. Elements created from palette and loaded from project files automatically get their isPro field set based on the PRO_ELEMENTS registry. Plan 02 will consume these APIs for visual indicators.
Output: proElements.ts registry, licenseSlice.ts store, useLicense.ts hook, updated BaseElementConfig type, wired element creation in App.tsx, wired project load in serialization.ts
</objective>
...
<task type="auto">
  <name>Task 3: Wire isProElement into element creation and project load</name>
  <files>src/App.tsx, src/services/serialization.ts</files>
  <action>
**Step 1: Wire isProElement into element creation (src/App.tsx)**
...
2. In `handleDragEnd` function, after each element is created via factory function (the big switch statement), add the isPro field before calling `addElement()`.

Find the section after the switch statement where `newElement` is created (around line 660-700). After the switch statement and before any modifications to `newElement` (like centering or layer assignment), add:
```typescript
// Set isPro flag based on element type
if (newElement && isProElement(newElement.type)) {
  newElement.isPro = true
}
```

This ensures every element created from the palette gets its isPro field populated.

**Step 2: Wire isProElement into project load (src/services/serialization.ts)**
...
2. In `deserializeProject` function, after schema validation succeeds and before returning the success result (around line 154-190), add a loop to populate isPro on all elements:
...
This ensures elements loaded from project files (which may not have isPro saved) get the correct isPro value based on the current PRO_ELEMENTS registry.
  </action>
  <verify>
Run `npx tsc --noEmit` to verify no type errors.
Run `npm run dev`, drag a Pro element (e.g., Scrolling Waveform) to canvas.
Check browser DevTools console: `useStore.getState().elements.find(e => e.type === 'scrollingwaveform')?.isPro` should return `true`.
Drag a Free element (e.g., Knob) and verify isPro is undefined.
Load an existing project and verify Pro elements have isPro: true after load.
  </verify>
  <done>
- Elements created from palette have isPro field set automatically via isProElement() check
- Elements loaded from project JSON have isPro field populated based on current PRO_ELEMENTS registry
- Pro elements have isPro: true, Free elements have isPro: undefined
  </done>
</task>
...
5. Elements created from palette have correct isPro value
6. Elements loaded from project have correct isPro value
</verification>
...
- Elements dragged from palette get isPro field set
- Elements loaded from project get isPro field set
