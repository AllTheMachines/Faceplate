...
<task type="auto">
  ...
  <files>src/components/elements/BaseElement.tsx, src/components/elements/Element.tsx, src/components/elements/index.ts</files>
  <action>
...
Update Canvas.tsx to import and render Element components:
```typescript
{elements.map((el) => (
